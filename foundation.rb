...
#*************************************************************************
def win_bet ( bet_amount , odds, player_purse)
...
def lose_bet ( bet_amount , player_purse)
...
if count >= 10
    social
    File.open("count.txt", "a") do |line|                                        #Write method to the betting ledger
        line.puts "\r" + "#{0}"
    end
else 
    File.open("count.txt", "a") do |line|                                        #Write method to the betting ledger
...
bet_hash = {1 => "Red or Black", 2 => "Odd or Even", 3 => "1-18", 4 => "19-36", 5=> "1st Dozen (1-12)", 6=> "2nd Dozen (13-24)", 7 => "3rd Dozen (25-36)", 8 => "Column 1", 9 => "Column 2", 10 => "Column 3", 11 => "Single number"}


system "clear"

#*************************************************************************
puts "You have selected a '#{bet_hash[bet_choice]}' type bet"
puts "\n"
puts "You currently have #{player_purse}"
puts "\n"
...
while (bet_amount > player_purse) || (bet_choice .negative? == true) || (bet_amount == 0)    #Limits user input to valid amount (no string, negative values or empty spaces)
    ...
    puts "You're betting $#{bet_amount}"
...
case bet_choice
    when 1                                                              
        ...
        if color_hash[color_choice]== wheel[ball_placement]                            #If the color_choice is equal to the associated color of the random number generated (This is taken from the hash)
            player_purse = win_bet(bet_amount, odds, player_purse)
        else 
            player_purse = lose_bet(bet_amount, player_purse)
       end
...
#*************************************************************************
    when 2
        odds = 1
                ...
                case odd_even
                when 2                                                             #1X payout    #If ball is even 
                    if ball_placement == 0                                          #Test for zero
                        player_purse = lose_bet(bet_amount, player_purse)
                    elsif (ball_placement %2 == 0)                                  #Test for even
                        player_purse = win_bet(bet_amount, odds, player_purse)                                                            #Displays new player purse
                    else                                                            #Odd
                        player_purse = lose_bet(bet_amount, player_purse)
                    end
                when 1 
